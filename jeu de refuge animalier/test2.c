...
int sauvegarder_animaux() {
    ...
    // ÉCRITURE COMPLÈTE dans le fichier horodaté
    ...
    // ÉCRITURE COMPLÈTE dans animaux.txt aussi
...
void supprimer_animal_du_fichier(int id_a_supprimer) {
    FILE *f_in = fopen("animaux.txt", "r");
    FILE *f_out = fopen("temp.txt", "w");
    FILE *f_adopt = fopen("adoptes.txt", "a"); // fichier des adoptés

    if (f_in == NULL || f_out == NULL || f_adopt == NULL) {
    ...
    // Lecture et copie
    while (fscanf(f_in, "%d;%d;%f;%d;%[^;];%[^\n]\n",
                  &a.id, &a.annee, &a.poids, (int*)&a.espece, a.nom, a.descrip) == 6) {
        if (a.id == id_a_supprimer) {
            fprintf(f_adopt, "%d;%d;%f;%d;%s;%s\n", a.id, a.annee, a.poids, a.espece, a.nom, a.descrip);
            trouvé = 1;
        } else {
            fprintf(f_out, "%d;%d;%f;%d;%s;%s\n", a.id, a.annee, a.poids, a.espece, a.nom, a.descrip);
        }
    }
    ...
    if (trouvé) {
        remove("animaux.txt");
        rename("temp.txt", "animaux.txt");
        printf("✅ Animal avec ID %d archivé dans adoptes.txt et supprimé de animaux.txt\n", id_a_supprimer);
    } else {
        remove("temp.txt");
        printf("❌ Aucun animal avec l'ID %d trouvé dans animaux.txt\n", id_a_supprimer);
    }
}
...
void adopter_animal() {
    ...
    for (int i = 0; i < nb_animal; i++) {
        if (refuge[i].id == id) {
            ...
            // 🟩 ENREGISTRE dans adoptes.txt AVANT de supprimer de refuge[]
            FILE *f_adopt = fopen("adoptes.txt", "a");
            if (f_adopt != NULL) {
                fprintf(f_adopt, "%d;%d;%f;%d;%s;%s\n", refuge[i].id, refuge[i].annee,
                        refuge[i].poids, refuge[i].espece, refuge[i].nom, refuge[i].descrip);
                fclose(f_adopt);
                printf("✅ Animal avec ID %d archivé dans adoptes.txt\n", id);
            }

            // 🔴 SUPPRIME dans le tableau refuge[]
            ...
            // 🟦 MET À JOUR animaux.txt sans l'animal supprimé
            sauvegarder_animaux();
            return;
        }
    }
...
void afficher_animaux_adoptes() {
    FILE *f = fopen("adoptes.txt", "r");
    if (f == NULL) {
        printf("Aucun animal adopté pour le moment.\n");
        return;
    }
...
int afficherMenu() {
    ...
    printf("                              |   8. 📜 Afficher les animaux adoptés                         |\n");
    ...
    switch (choix) {
        ...
        case 8:
		printf("→ [Afficher les animaux adoptés] 📜\n");
		afficher_animaux_adoptes();
		break;
